...
const JoinCreateChat = () => {
  ...
  const { room, setRoom, setRoomId, setCurrentUser, setConnected } =
  // const { roomId, setRoomId, setCurrentUser, setConnected } =
    useChatContext();
  ...
  async function joinChat() {
    if (validateForm()) {
      ...
      try {
        const room1 = await joinChatApi(detail.roomId);
        toast.success("joined..");
        setCurrentUser(detail.userName);
        // setRoomId(room1.roomId);
        setRoom(room1);
        // console.log("room is(join) : "+JSON.stringify(room, null, 2));
        setConnected(true);
        navigate("/chat");
      } catch (error) {
        if (error.status == 400) {
  ...
  async function createRoom() {
    if (validateForm()) {
      ...
      try {
        ...
        setCurrentUser(detail.userName);
        setRoom(response);
        // console.log("room is(create) : "+room);
